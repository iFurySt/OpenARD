...
- Treat detached compact JWS verification as explicit operator trust-anchor verification.
  ...
  documents and use OKP/Ed25519 `verificationMethod[].publicKeyJwk` keys. SPIFFE,
  certificate, OIDC, and non-`did:web` DID discovery are not implemented.
...
- SPIFFE, certificate, OIDC, non-`did:web` DID, and broader trust-anchor discovery depth
  for MVP.
