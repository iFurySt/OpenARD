...
| --- | --- | --- | --- |
| Product surface | C | CLI and registry can import catalogs, crawl well-known catalogs, onboard MCP/A2A/Skill artifacts, list and remove entries, export catalogs, search, browse, explore, and manage remote admin APIs. Combined, CLI-only, and server-only entrypoints are available. Admin lifecycle status can hide inactive entries from public discovery. Policy and richer approval workflows are still missing. | Define publish/update approval flow semantics and audit events. |
